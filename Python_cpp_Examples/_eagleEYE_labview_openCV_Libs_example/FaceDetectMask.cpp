...
EXTERN_C void QwaveCV_FaceDetectMask(lvError * errorHandle, CVImageHandle *hImage, CVImageHandle *hMask) {
    ...
    IMAQCV_ReturnOnPreviousError(errorHandle);
    try
    {
        if (!hImage) {
            IMAQCV_ThrowCVError(ERR_NULL_POINTER);
        }

		double min_face_size=20;
		double max_face_size=200;

        Mat image = IMAQCV_LVToMat(hImage);
		Mat mask = IMAQCV_LVToMat(hMask);
    ...
    IMAQCV_ProcessCVError(error, errorHandle);
}
